...
class Line:
    ...
    def process_message(self, message):
        ...
        topic = message.topic()
        if topic == "org.cta.stations.table.v1": # Set the conditional correctly to the stations Faust Table
            try:
                value = json.loads(message.value())
                self._handle_station(value)
            except Exception as e:
                logger.fatal("bad station? %s, %s", value, e)
        elif "org.cta.station.arrivals.v1" in topic: # Set the conditional to the arrival topic
