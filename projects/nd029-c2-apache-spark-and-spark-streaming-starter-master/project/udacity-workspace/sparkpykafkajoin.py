...
# cast the value column in the streaming dataframe as a STRING 
eventsStreamingDF = eventsStreamingDF.selectExpr("cast(key as string) key", "cast(value as string) value")
...
eventsStreamingDF.withColumn("value",from_json("value", customerRiskSchema)) \
        .select(col('value.*')) \
...
# verify data by printing the results in the console
riskScoreByBirthYear.writeStream.outputMode("append").format("console").start().awaitTermination()